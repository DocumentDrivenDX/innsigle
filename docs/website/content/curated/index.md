---
title: Innsigle
nav: home
weight: 0
description: Craft seal for content mash bills — composition, not verdict.
---

# A craft seal for how work was made

**Innsigle** (say **INN-siggle**, rhymes with *single*) is a visible mark plus a
**mash bill**—the recipe of production: human-authored, mixed, or model-primary—with
optional house or person signatures. Same seal family for both poles. Not a purity
score. Not an AI detector.

## Two jobs, one system

### Docs (model-primary, proud)

Long-lived documentation often drafted with models. Seal a mash bill that *names*
tools (Claude, sloptimizer, …) and, when you want standing, sign with a house key.

### Social (human-authored, clear)

Short posts strip file metadata. Use the same seal family with a human cue and a
link out. Visibility first; per-post crypto optional.

## What verification answers

When a claim is signed: **did this issuer seal this mash bill for these content
bytes?** It does not answer whether the prose is true, pure, or free of AI.

## Start

- [Why Innsigle](why/) — thesis and audience
- [Use](use/) — CLI, mash bill, verify, marks
- [Reference](reference/) — generated specs from `docs/helix/`
- [Dogfood sample](dogfood/) — signed model-primary page
- [Non-goals](non-goals/) — category boundaries
